...
fwloader_fw_ver_init(void)
{
...
#if defined(CONFIG_BOOTLOADER_MCUBOOT)
    fw_img_print_image_info(PM_ID(mcuboot_secondary), &g_fw_img_fw_ver, &g_fw_img_hw_rev);
    ...
    LOG_INF(
        ...
        APP_COMMIT_STRING);
#else
    LOG_INF("### Ruuvi FwLoader: Version: %s, build: %s", fwloader_fw_ver_get(), STRINGIFY(APP_BUILD_VERSION));
#endif
    LOG_INF(
