...
static bool g_fwloader_led_in_pwm_mode;
static K_MUTEX_DEFINE(g_fwloader_led_mutex);
...
#if (IS_ENABLED(CONFIG_RUUVI_AIR_PINHOLE_LED_PWM))
static void
fwloader_led_deinit_pwm(const struct pwm_dt_spec* p_pwm_led_spec)
{
...
{
#if (IS_ENABLED(CONFIG_RUUVI_AIR_PINHOLE_LED_PWM))
    fwloader_led_deinit_pwm(&pwm_led_red);
    fwloader_led_deinit_pwm(&pwm_led_green);
    g_fwloader_led_in_pwm_mode = false;
#endif // (IS_ENABLED(CONFIG_RUUVI_AIR_PINHOLE_LED_PWM))
#if (!IS_ENABLED(CONFIG_RUUVI_AIR_PINHOLE_LED_NONE))
...
#if (IS_ENABLED(CONFIG_RUUVI_AIR_PINHOLE_LED_PWM))
    fwloader_led_deinit_gpio(&led_red);
    fwloader_led_init_pwm_gpio("Red", &led_red, &pwm_led_red);
    fwloader_led_deinit_gpio(&led_green);
    fwloader_led_init_pwm_gpio("Green", &led_green, &pwm_led_green);
    g_fwloader_led_in_pwm_mode = true;
#endif // (IS_ENABLED(CONFIG_RUUVI_AIR_PINHOLE_LED_PWM))
}
...
{
    k_mutex_lock(&g_fwloader_led_mutex, K_FOREVER);
}

void
fwloader_led_unlock(void)
{
    k_mutex_unlock(&g_fwloader_led_mutex);
}
