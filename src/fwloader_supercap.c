...
fwloader_on_supercap_active(void)
{
...
static void
fwloader_isr_cb_supercap_active(const struct device* dev, struct gpio_callback* cb, uint32_t pins)
{
    (void)dev;
    (void)cb;
    (void)pins;

    fwloader_on_supercap_active();
}

void
fwloader_supercap_init(void)
{
    LOG_INF("fw_loader: Set up GPIO SUPERCAP_ACTIVE");
    const gpio_flags_t extra_flags = 0;
    fwloader_gpio_input_init(
        &g_supercap_active,
        extra_flags,
        &g_supercap_active_isr_gpio_cb_data,
        &fwloader_isr_cb_supercap_active,
